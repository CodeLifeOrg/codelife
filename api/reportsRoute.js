module.exports = function(app) {
  ...
  app.get("/api/reports/codeblocks/all", (req, res) => {
    ...
    const q = "select reports.id, reports.uid, reports.reason, reports.comment, reports.report_id, reports.type, users.username, users.email, users.name, codeblocks.snippetname as filename from reports, users, codeblocks where reports.report_id = codeblocks.id AND codeblocks.uid = users.id AND reports.type = 'codeblock'";
  ...
  app.get("/api/reports/projects/all", (req, res) => {
    ...
    const q = "select reports.id, reports.uid, reports.reason, reports.comment, reports.report_id, reports.type, users.username, users.email, users.name, projects.name as filename from reports, users, projects where reports.report_id = projects.id AND projects.uid = users.id AND reports.type = 'project'";
    db.query(q, {type: db.QueryTypes.SELECT}).then(u => res.json(u).end());
