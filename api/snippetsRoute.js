module.exports = function(app) {
  ...
  app.get("/api/snippets/allothers", (req, res) => {
    ...
    const q = "SELECT snippets.id, snippets.snippetname, snippets.studentcontent, snippets.likes, snippets.previewblob, snippets.lid, snippets.uid, users.username FROM snippets, users WHERE users.id = snippets.uid AND users.id != '" + req.user.id + "'";
    db.query(q, {type: db.QueryTypes.SELECT}).then(u => res.json(u).end());
