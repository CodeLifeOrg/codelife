...
class IslandLink extends Component {
  ...
  render() {
    ...
    if (lesson.isNext || next || lesson.isDone || done) {
      ...
      return <Link to={`/lesson/${lesson.id}`} className={ css } key={ lesson.id } style={{height: `${width * 0.75}px`, width: `${width}px`}}>
        <div className="graphic" style={{backgroundImage: `url('/islands/${ lesson.id }.png')`, height: `${width * 0.75}px`, width: `${width}px`}}></div>
    ...
    return <div className="islandLink" key={ lesson.id } style={{height: `${width * 0.75}px`, width: `${width}px`}}>
      <div className="graphic" to={`/lesson/${lesson.id}`} style={{backgroundImage: `url('/islands/${ lesson.id }.png')`, height: `${width * 0.75}px`, width: `${width}px`}}></div>
      <div className={ `pt-popover pt-tooltip ${ lesson.id }` } style={{marginTop: `${width * 0.25}px`}}>
        <div className="pt-popover-content">
